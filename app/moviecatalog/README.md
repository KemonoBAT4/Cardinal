...
- Act on the database:
    - **`./database.sh <project_name> --reset`** resets the database
    - **`./database.sh <project_name> --migrate`** migrates the database without removing the data
    - **`./database.sh <project_name> --shell`** opens the database shell to execute action on the db of the project

- Set up basic data (edit the setup function in `cli.py` for every specifi project)
    - **`./run.sh <project_name> setup <arguments>`**
